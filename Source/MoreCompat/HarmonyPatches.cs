        ...
        public static class Pawn_VarietyTracker_TrackVarieties
        {
            ...
            private static void Postfix(ref Pawn_VarietyTracker pawnRecord, Pawn ingester, Thing foodSource)
            {
                ...
                if (extension != null)
                {
                    ...
                    if (ModSettings_VarietyMatters.betterArchotech)
                ...
                var condiments = !ModSettings_VarietyMatters.betterGourmet || stackByCondimentType == null ? null : (foodSource as ThingWithComps)?.AllComps.FirstOrDefault(x => stackByCondimentType.IsInstanceOfType(x));
                var baseExpectation = !ModSettings_VarietyMatters.fixDesserts ? -1 : (int)VarietyExpectation.GetBaseVarietyExpectation(ingester);
